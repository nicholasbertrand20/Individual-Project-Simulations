% --- This simulation is used to display Fig. 4.2 ---

clc; clear; close all;

% Warehouse Dimension Parameters
warehouse_width = (11:6:143);    % Number of columns (horizontal width of the warehouse grid in meters)
...
Nmax = 1e5;                      % Number of simulations
...
a2 = 3.5;
a3 = 4;
b = 3;                           % Basic Path Loss Exponent of Interfering Signal
...
gam_thr = 10^(3/10);             % 3 dB SINR threshold to read
lambda_int = 2 / 100000;         % Density of interferers (m^-2)
...
P_out_3_67 = zeros(length(warehouse_width), Nmax);
mean_P_out_3_67 = zeros(length(warehouse_width), 1);
P_out_2 = zeros(length(warehouse_width), Nmax);
mean_P_out_2 = zeros(length(warehouse_width), 1);
P_out_3 = zeros(length(warehouse_width), Nmax);
mean_P_out_3 = zeros(length(warehouse_width), 1);
...
for idx = 1:length(warehouse_width)
    ...
    % Assign each device to the relay in the same row
    ...
    for iter = 1 : Nmax
        ...
        % SINR calculation
        SINR_3_67(idx, iter) = ( S_t * user_distance_to_relay.^(-a1) * 10^-(W_out/10) * 10^-(W_in*k/10) * X_s_out * X_m_1 ) ./ ... 
        ( P_int + N_0 );
        SINR_2(idx, iter) = ( S_t * user_distance_to_relay.^(-a2) * 10^-(W_out/10) * 10^-(W_in*k/10) * X_s_out * X_m_1 ) ./ ...
        ( P_int + N_0 );
        SINR_3(idx, iter) = ( S_t * user_distance_to_relay.^(-a3) * 10^-(W_out/10) * 10^-(W_in*k/10) * X_s_out * X_m_1 ) ./ ...
        ( P_int + N_0 );

        % Outage probability calculation
        P_out_3_67(idx, iter) = SINR_3_67(idx, iter) <= gam_thr;
        P_out_2(idx, iter) = SINR_2(idx, iter) <= gam_thr;
        P_out_3(idx, iter) = SINR_3(idx, iter) <= gam_thr;
    ...
    mean_P_out_3_67(idx) = max(mean(P_out_3_67(idx, :), 2), 1e-5);
    mean_P_out_2(idx) = max(mean(P_out_2(idx, :), 2), 1e-5);
    mean_P_out_3(idx) = max(mean(P_out_3(idx, :), 2), 1e-5);

end

figure;
semilogy(warehouse_width, mean_P_out_2, '-^', 'LineWidth', 2, 'Color', 'r');
hold on;
semilogy(warehouse_width, mean_P_out_3, '-x', 'LineWidth', 2, 'Color', 'g');
semilogy(warehouse_width, mean_P_out_3_67, '-o', 'LineWidth', 2, 'Color', 'b');
% Set Labels with Larger Font
xlabel('Warehouse Width w_w (m)', 'FontSize', 13);
ylabel('Outage Probability of DF Relay', 'FontSize', 13);
